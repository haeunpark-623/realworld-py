...
- **언어/런타임**: Python 3.11 (백엔드) + Node.js 20 LTS (프론트엔드 빌드)
- **패키지 매니저**: `uv` (Astral, Python) + `pnpm` 9 (Node)
...
# 또는: corepack enable && corepack prepare pnpm@9.15.4 --activate
```
...
본 프로젝트는 풀스택 monorepo + FE/BE가 다른 stack (`pnpm`/`Vite` ↔ `uv`/`FastAPI`) 조합이므로 **§1.5.1 패턴 (e) 워크스페이스별 .env 완전 분리** 채택.
...
# frontend/.env 의 VITE_API_BASE_URL은 기본 http://localhost:8000/api 유지
...
- **기대 출력 (frontend)**: `VITE v5.x.x  ready in NNN ms` + `➜  Local:   http://localhost:5173/`
...
| lockfile | `backend/uv.lock` + `frontend/pnpm-lock.yaml` (★2) | 의존성 추가/변경 | 의존성 도입 이슈 |
| 설치/seed scripts | `backend/scripts/seed_articles.py` | seed 데이터 변경 | seed 변경 이슈 |
| 부팅 명령 | 본 LOCAL.md §3.1 + `backend/pyproject.toml` (uv scripts) + `frontend/package.json scripts.dev` | 명령 변경 | 명령 변경 이슈 |
...
> **★2 lockfile footnote**: stack이 다르므로 lockfile 2종(`uv.lock` Python + `pnpm-lock.yaml` Node) 공존. 둘 다 commit + AI 게이트가 누락된 lockfile 갱신을 BLOCK.
...
**N/A** — 본 프로젝트는 Gradle/Maven 등 parent-aware 빌드 도구 미사용. uv + pnpm 조합으로 워크스페이스별 직호출.
