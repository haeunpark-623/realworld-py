...
## 기술 스택
> 게이트 B(`/flow-init` v0.2) 사전 결정 + 게이트 C(`/flow-design`) 잔여 확정. 정본은 `docs/planning/05-prd/05-prd.md §1.1` + `02-feasibility §6.1`.

- 프론트엔드: **React + Vite + TypeScript (SPA)** — 스타일링 솔루션은 게이트 C(ADR-0038) 결정 예정 (Tailwind / CSS Modules / styled-components 중)
- 백엔드: **FastAPI + Python 3.11 + Pydantic + SQLAlchemy** — DB는 게이트 C 결정 (SQLite vs PostgreSQL). 인증은 JWT + bcrypt (`passlib[bcrypt]` + `pyjwt` 또는 `python-jose`)
- 인프라: **로컬 dev 1 profile만** (RFP §NFR-06, 단일 환경 운영 명시). dev/stg/prod 3 profile은 N/A. CI는 GitHub Actions pytest 워크플로 1개
